...
class AutoEnv:
    ...
    def reset(self, offset: int = None, egoid: int = None, start: int = None, traj_idx: int = 0):
        if offset is None:
    ...
    def veh_state(self):
        x = self.ego_veh.state.posG.x
        y = self.ego_veh.state.posG.y
        theta = self.ego_veh.state.posG.theta
        v = self.ego_veh.state.v
        return x, y, theta, v
    ...
    def front_state(self):
        ...
        if fore_fore_M.ind is not None: # fornt car exists
            x = scene[fore_fore_M.ind].state.posG.x
            y = scene[fore_fore_M.ind].state.posG.y
            theta = scene[fore_fore_M.ind].state.posG.theta
            v = scene[fore_fore_M.ind].state.v
        else:
            x = 0
            y = 0
            theta = 0
