...
class MultiAgentAutoEnv:
    ...
    def reset(self, dones: list = None, offset: int = None, random_seed: int = None, egoid: int = None, traj_idx: int = None, start: int = None):
        if offset is None:
        ...
        if traj_idx is None or egoid is None or start is None:
            ...
            print("===sampling===")
            sys.exit("entering the sampling, however it very unlikely")
        ...
        # print("maxium horizon is:", self.h)
    ...
    # calculate vehicle state for safety monitor
    def veh_state(self):
        # print("==== start get ego vehicle state=======")
        x = self.ego_vehs[0].state.posG.x
        y = self.ego_vehs[0].state.posG.y
        theta = self.ego_vehs[0].state.posG.theta
        v = self.ego_vehs[0].state.v
        idnum = self.ego_vehs[0].id
        return x, y, theta, v, idnum

    # calculate the front vehicle state 
    # def front_state(self):
        # # print("==== start get front vehicle state=======")
        # veh_idx = self.scene.findfirst(self.egoids[0])
        # pastframe = 0 # think this is looking back how long the frame
        # rec = self.rec
        # scene = rec[pastframe]
        # # for veh_idx in range(scene.n):
        # #     if scene[veh_idx] is not None:
        # #         print("car{} x,y from front state:".format(scene[veh_idx].id),scene[veh_idx].state.posG.x, scene[veh_idx].state.posG.y)
        # roadway = self.roadway
        # ego_vel = scene[veh_idx].state.v
        # vtpf = "Front"
        # vtpr = "Rear"
        # if self.frontid is None:
        #     fore_M = get_neighbor_fore_along_lane_2(scene, veh_idx, roadway, vtpf, vtpr, vtpf)
        #     if fore_M.ind is not None: # front car exists
        ...
        #         print("the fronveh {} at segment {} and lane {}".format(scene[fore_M.ind].id, scene[fore_M.ind].state.posF.roadind.tag.segment, scene[fore_M.ind].state.posF.roadind.tag.lane))
        #         print("the front of ego {} is car {}, closet distance is{}".format(scene[veh_idx].id, scene[fore_M.ind].id, fore_M.delta_s))
        #         x = scene[fore_M.ind].state.posG.x
        #         y = scene[fore_M.ind].state.posG.y
        #         theta = scene[fore_M.ind].state.posG.theta
        #         v = scene[fore_M.ind].state.v
        #         self.frontid = scene[fore_M.ind].id
        #     else:
        #         x = 0
        #         y = 0
        #         theta = 0
        ...
        #         veh = scene[i]
        #         if veh.id == self.frontid:
        #             for_found = True
        #             break
        #     if for_found == True: # the original front car exist 
        #         print("the egoveh {} is at segment {} and lane {}".format(scene[veh_idx].id, scene[veh_idx].state.posF.roadind.tag.segment, scene[veh_idx].state.posF.roadind.tag.lane))
        #         print("the fronveh {} at segment {} and lane {}".format(veh.id, veh.state.posF.roadind.tag.segment, veh.state.posF.roadind.tag.lane))
        #         x = veh.state.posG.x
        #         y = veh.state.posG.y
        #         theta = veh.state.posG.theta
        #         v = veh.state.v
        #     else:
        #         sys.exit("the front car disappeared in the scene")
        # return x, y, theta, v
    ...
    def check_segment(self):
        # print("==== start get front vehicle state=======")
        print("now the ego vehicle is {}, its segment {}, and lane {}".format(self.ego_vehs[0].id, self.ego_vehs[0].state.posF.roadind.tag.segment, self.ego_vehs[0].state.posF.roadind.tag.lane))

        seg = self.ego_vehs[0].state.posF.roadind.tag.segment
        lane = self.ego_vehs[0].state.posF.roadind.tag.lane
        return seg

    def get_ori_acc(self):
        # print("==== start get front vehicle state=======")
        delta_t = 0.1 # record time is 0.1
        scene_length = max_n_objects(self.trajdatas)
        scene = Frame()
        scene_past = Frame()
        scene.init(scene_length)
        scene_past.init(scene_length)
        scene = get_scene(scene, self.trajdatas[self.traj_idx], self.t)
        scene_past = get_scene(scene_past, self.trajdatas[self.traj_idx], self.t-1)
        for (i, egoid) in enumerate(self.egoids):
            vehidx = scene.findfirst(egoid)
            vehidxp = scene_past.findfirst(egoid)
            orig_vehs = scene[vehidx]
            orig_vehs_p = scene_past[vehidxp]
            # print("current frame step is:", self.t)
            # print("id and idp =", orig_vehs.id, orig_vehs_p.id)
            # track the original vehicle for validation / infos purposes
            # print("orig v {} and past v{}".format(orig_vehs.state.v, orig_vehs_p.state.v))
            acc = (orig_vehs.state.v - orig_vehs_p.state.v) / delta_t
            break
        # sys.exit("hiyayay")
        return acc
