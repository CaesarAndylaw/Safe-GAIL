...
def get_neighbor_fore_along_lane_1(scene: Frame, roadway: Roadway, tag_start: LaneTag, s_base: float,
    ...
    while dist_searched < max_distance_fore:
        ...
        # print("there are {} cars in current frame".format(scene.n))
        # print("==========the target tag is segment{} and lane{}".format(tag_target.segment, tag_target.lane))
        for i in range(scene.n):
            veh = scene[i]
            # print("==look we see car{}".format(veh.id))
            if i != index_to_ignore:
                # print("the current is segment{} and lane{}".format(veh.state.posF.roadind.tag.segment, veh.state.posF.roadind.tag.lane))
                s_adjust = None
                # if veh.state.posF.roadind.tag == tag_target:
                #     s_adjust = 0.0

                if veh.state.posF.roadind.tag.segment == tag_target.segment and veh.state.posF.roadind.tag.lane == tag_target.lane:
                    s_adjust = 0.0
                    ...
                    print("that's very weird, no corner cases")
                    sys.exit("between sgment hi??")
                ...
                if s_adjust is not None:
                    ...
                    # print("s_valid - s_base =", s_valid - s_base)
                    if dist_valid >= 0.0:
