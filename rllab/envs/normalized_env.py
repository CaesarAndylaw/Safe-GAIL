...
class NormalizedEnv(ProxyEnv, Serializable):
    ...
    def veh_state(self):
        vs = self._wrapped_env.veh_state()
        return vs

    def front_state(self):
        fs = self._wrapped_env.front_state()
        return fs

    def check_segment(self):
        cs = self._wrapped_env.check_segment()
        return cs

    def get_ori_acc(self):
        cs = self._wrapped_env.get_ori_acc()
        return cs
    ...
    def step(self, action):
        ...
        # print(self._scale_reward, reward)
